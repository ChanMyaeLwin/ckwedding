                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('documents.update',$document->id) }}" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
                            ...
                            @if($document->document_status != 1 && auth()->user()->can('change_to_previous_status') &&
                                ...
                                ($document->document_status == 4 && auth()->user()->can('update-document-ch-complete')) ||
                                ($document->document_status == 6 && auth()->user()->can('update-document-mm-complete')) ||
                    ...
                    <div class="col-lg-12">
                        <div class="d-flex flex-wrap flex-wrap align-items-center justify-content-between mb-4">
                            <div>
                                <h4 class="mb-3"> {{ __('product.add_product_list')}} </h4>
                                <p class="mb-0">{{ __('product.check_product')}}</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-12">
                            ...
                            <table class="table mb-0 tbl-server-info" id="product_list_by_document">
                                <thead class="bg-white text-uppercase">
                                    <tr class="ligth ligth-data">
                                        <th>{{__('product.product_code')}}</th>
                                        <th>{{__('product.product_name')}}</th>
                                        <th>
                                            @if($document->document_type == 1)
                                            Return {{__('product.qty')}}
                                            @else
                                            Exchange Qty</th>
                                        @endif
                                        <th>Br.Mgr {{__('product.qty')}}</th>
                                        <th>Mer. {{__('product.qty')}}</th>
                                        <th>RG Out {{__('product.qty')}}</th>
                                        <th>RG In {{__('product.qty')}}</th>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form id="product_form" action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <input type="hidden" name="document_m_remark" id="document_m_remark" value="{{$document->merchandising_remark}}" /> 
        ...
        if ($case == 'bm-approve') {
            ...
            Swal.fire({  
                ...
                if (result.isConfirmed){
                    ...
                    if(!$('#document_m_remark').val())
                    {
                        Swal.fire({
                            icon: 'warning',
                            title: "{{ __('message.warning') }}",
                            text: "{{ __('message.need_mer_remark') }}",
                            confirmButtonText: "{{ __('message.ok') }}",
                        });
                    }else{
                        if(result == false){
                ...
                render: function(data, type, row) {
                    return `  
                            <div class="d-flex align-items-center list-action">
                                ...
                                @if(($document->document_status == 1 || $document->document_status == 2 || $document->document_status == 6 || $document->document_status == 8) && auth()->user()->can('edit-product'))
                                <a class="badge bg-success mr-2" data-product_id="${row.id}" title="Edit" id="edit"  href="#"><i class="ri-pencil-line mr-0"></i></a>
                                @endif
                                @if($document->document_status == 1  && auth()->user()->can('edit-product'))
